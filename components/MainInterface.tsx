...
const MainInterface: React.FC = () => {
  ...
  const getIcon = (iconName: string, active: boolean) => {
    const className = `w-5 h-5 transition-colors duration-300 ${
      active ? "text-claude-accent" : "text-claude-muted"
    }`;
    switch (iconName) {
      case "Apple":
    ...
    <div className="min-h-screen w-full bg-claude-bg text-claude-text selection:bg-claude-accent/30 selection:text-white overflow-x-hidden font-sans flex flex-col">
      ...
      <div className="fixed top-4 right-4 z-50">
        ...
        >
          <Globe
            size={14}
            className="text-claude-muted group-hover:text-claude-accent transition-colors"
          />
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 py-12 sm:py-20 flex flex-col items-center gap-8 flex-1">
        ...
        <header className="text-center space-y-5 w-full" role="banner">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-claude-card border border-claude-border/60 shadow-sm mb-4">
              <Sparkles
                size={14}
                className="text-claude-accent animate-pulse"
                aria-hidden="true"
              />
            ...
            <p
              className={`mt-6 text-claude-muted text-sm sm:text-base max-w-md mx-auto leading-relaxed ${
        ...
        >
          <div
            className="grid grid-cols-3 gap-2 p-1.5 bg-claude-card/80 backdrop-blur-md border border-claude-border rounded-2xl shadow-lg"
            role="tablist"
            aria-label="選擇作業系統平台"
          >
            {PLATFORM_DATA.map((platform) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col sm:flex-row items-center justify-center gap-2 py-3 sm:py-3 px-2 rounded-xl transition-all duration-300 ${
                    isActive
                      ? "text-white"
                      : "text-claude-muted hover:text-white/80"
                  }`}
                >
                  {isActive && (
                    <motion.div
                  ...
                  <span className="relative z-10">
                    {getIcon(platform.iconName, isActive)}
                  </span>
                  <span
            ...
            >
              <div className="flex items-center justify-between px-2">
                <h2 className="text-lg font-medium text-white/90 flex items-center gap-2">
                  <span
                    className="w-1.5 h-1.5 rounded-full bg-claude-accent shadow-[0_0_10px_rgba(217,119,87,0.8)]"
                    aria-hidden="true"
                  ></span>
